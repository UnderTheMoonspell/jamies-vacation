...
export const getObjectDiff = (o1: any, o2: any) => {
  return Object.keys(o1).reduce((diff, key) => {
    return JSON.stringify(o1[key]) === JSON.stringify(o2[key]) ? diff : { ...diff, [key]: o2[key] }
  }, {})
}
...
export const generateId = () => Math
  .random()
  .toString(36)
  .replace(/[^a-z]+/g, '')
  .substr(0, 5)

export const formattedDate = (date = new Date(Date.now())) => moment(date).format('yyyy-MM-DD HH:mm')
